...
fdd_jn2 <- SharedData$new(fdd_jn)

#plot cumulative fdd
gg_fdd <- ggplot() + 
  geom_line(data = fdd_jn2, aes(x = as.Date(wy_doy, origin = "2018-10-01"), y = fdd, group = year, col = severity), size = 0.25, alpha = (0.75)) +
...
filter_plotly <- ggplotly(gg_fdd)

filter <- bscols(
  filter_select("id", "Select a single year", fdd_jn2, ~year),
...
sd_jn2 <- SharedData$new(sd_jn)

#plot observed and historic sd
gg_sd <- ggplot() + 
  geom_line(data = sd_jn2, aes(x = as.Date(wy_doy, origin = "2018-10-01"), y = snow_grnd, group = year, col = severity), size = 0.25, alpha = 0.75) +
  geom_text(data = sd_date, aes(x = as.Date(b_doy + 30, origin = "2018-10-01"), y = snow_grnd, group = year, label = year), col = "black", size = 2, alpha = 0.75) +
...
filter_plotly <- ggplotly(gg_sd)

filter <- bscols(
  filter_select("id", "Select a single year", sd_jn2, ~year),
  #filter_checkbox("id", "Select a single year", q_jn2, ~year, inline = T),
  filter_plotly,
  widths = c(12, 12)
)
...
# ####----Gathe WISKI Data for Current Year----####
#   
# ##download NRT Moosonee CLIM data from Wiski using kiwisR
# #get and store station information for Moosonee
# sta <- ki_station_list(hub = 'swmc', search_term = "*Moosonee RCS*")
